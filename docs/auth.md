...
`protectedRoutes`를 지정하지 않으면(빈 배열), **publicRoutes 외 모든 라우트가 자동으로 보호**됩니다. 대부분의 경우 `publicRoutes`만 설정하면 충분합니다.

특정 라우트만 보호하고 싶다면 `protectedRoutes`를 명시할 수 있습니다:

```javascript
const router = new ViewLogicRouter({
    auth: true,
    loginRoute: 'login',
    protectedRoutes: ['profile', 'admin/*'],  // 이 라우트만 보호
});
```
...
> **주의**: `localStorage.getItem('authToken')` 등으로 토큰을 직접 접근하지 마세요. `authStorage` 설정에 따라 저장소가 달라지므로 반드시 `this.getToken()` / `this.getRefreshToken()`을 사용하세요.
...
const router = new ViewLogicRouter({
    ...
    // protectedRoutes 생략 → publicRoutes 외 전부 보호
    // authFunction 불필요
});
```
